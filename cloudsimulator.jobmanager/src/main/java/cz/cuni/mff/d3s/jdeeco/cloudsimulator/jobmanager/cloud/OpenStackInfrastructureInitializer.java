...
public class OpenStackInfrastructureInitializer {
	...
	private Tenant getTenant() {
		if (this.tenant == null) {
			this.tenant = getClient().identity().tenants()
					.getByName(OpenStackInfrastructureInitializerParameters.TENANT_NAME);
		}
		return tenant;
	}

	private <T> T getFirst(List<T> list, Predicate<T> predicate) {
	 ...
	 * return os.networking().network().list().stream().filter(x ->
	 * x.getName().equals(networkName)).findFirst().get(); }
	 */

	private void attachVolume(String volumeName, String serverName) {
		Volume volume = getFirst(getClient().blockStorage().volumes().list(), x -> x.getName() == volumeName);
		Server server = getFirst(getClient().compute().servers().list(), x -> x.getName() == serverName);
		
		getClient().compute().servers().attachVolume(server.getId(), volume.getId());
	}
	 ...
	 * create inner network Network innerNetwork = os .networking() .network()
	 * .create
	 * (Builders.network().name(OpenStackInfrastructureInitializerParameters
	 * .INNER_NETWORK_NAME) .tenantId(tenant.getId()).build());
	 * 
	 * // create inner network subnet Subnet innerNetworkSubnet = os
	 * .networking() .subnet()
	 * .create(Builders.subnet().name(OpenStackInfrastructureInitializerParameters
	 * .INNER_NETWORK_SUBNET_NAME)
	 * .networkId(innerNetwork.getId()).tenantId(tenant
	 * .getId()).ipVersion(IPVersionType.V4) .addPool("192.168.0.1",
	 * "192.168.254.254").cidr("192.168.0.0/16").build());
	 * 
	 * // create router Network outerNetwork = getNetworkByName(os,
	 * OpenStackInfrastructureInitializerParameters.OUTER_NETWORK_NAME); Router
	 * router = os .networking() .router()
	 * .create(Builders.router().name(OpenStackInfrastructureInitializerParameters
	 * .ROUTER_NAME)
	 * .adminStateUp(true).externalGateway(outerNetwork.getId()).build());
	 * 
	 * // attach an external interface os.networking().router()
	 * .attachInterface(router.getId(), AttachInterfaceType.SUBNET,
	 * innerNetworkSubnet.getId());
	 * 
	 * // create security groups and rules SecGroupExtension httpSecurityGroup =
	 * os.compute().securityGroups().create("http", "http"); os.networking()
	 * .securityrule()
	 * .create(Builders.securityGroupRule().securityGroupId(httpSecurityGroup
	 * .getId()).direction("ingress")
	 * .protocol("tcp").portRangeMin(80).portRangeMax(80).build());
	 * 
	 * SecGroupExtension sshSecurityGroup =
	 * os.compute().securityGroups().create("ssh", "ssh"); os.networking()
	 * .securityrule()
	 * .create(Builders.securityGroupRule().securityGroupId(sshSecurityGroup
	 * .getId()).direction("ingress")
	 ...
	 * Builders.flavor().name("Job Manager Flavor").ram(8192
	 * ).vcpus(6).disk(240).build(); flavor =
	 * os.compute().flavors().create(flavor);
	 * 
	 * // get image Image jobManagerImage = os.compute().images()
	 * .get(OpenStackInfrastructureInitializerParameters
	 * .JOB_MANAGER_TEMPLATE_NAME);
	 * 
	 * // create a job manager model object ServerCreate jobManagerCreate =
	 * Builders.server()
	 * .name(OpenStackInfrastructureInitializerParameters.JOB_MANAGER_NAME
	 * ).flavor(flavor.getId()) .image(jobManagerImage.getId()).build();
	 * 
	 * // boot the server Server server =
	 * os.compute().servers().bootAndWaitActive(jobManagerCreate, 1000 * 60 *
	 * 10);
	 * 
	 * // public IP Network outerNetwork = getNetworkByName(os,
	 * OpenStackInfrastructureInitializerParameters.OUTER_NETWORK_NAME);
	 * NetFloatingIP publicIP = os.networking().floatingip()
	 * .create(Builders.netFloatingIP
	 * ().floatingNetworkId(outerNetwork.getId()).build());
	 * 
	 * os.compute().floatingIps().addFloatingIP(server,
	 * publicIP.getFloatingIpAddress()); }
	 */
	/*
	 * private void createInitialWorkers(OSClient os, Tenant tenant) {
	 * 
	 * // create flavor Flavor flavor =
	 * Builders.flavor().name(OpenStackInfrastructureInitializerParameters
	 * .WORKER_FLAVOR_NAME) .ram(2048).vcpus(2).disk(120).build(); flavor =
	 * os.compute().flavors().create(flavor);
	 * 
	 * for (int i = 0; i <
	 * OpenStackInfrastructureInitializerParameters.INITIAL_WORKER_COUNT; i++) {
